...
async function search({ member, guild, channel }, query) {
  ...
  const trackList = res.tracks || res.data?.tracks || (res.data ? [res.data] : []);
  const loadType = trackList.length > 0 ? res.loadType : "NO_MATCHES";
  switch (loadType) {
    case "LOAD_FAILED":
    case "error":
      guild.client.logger.error("Search Exception", res.exception || res.data);
      return "🚫 There was an error while searching";
    ...
    case "playlist":
      ...
      embed
        ...
        .setDescription(res.playlistInfo?.name || res.data?.info?.name || "Playlist")
        .addFields(
          {
            name: "Enqueued",
            value: `${trackList.length} songs`,
            inline: true,
          },
          {
            ...
            value:
              "`" +
              prettyMs(
                trackList.map((t) => t.info.length).reduce((a, b) => a + b, 0),
                { colonNotation: true }
              ) +
              "`",
            inline: true,
    ...
    case "search": {
      ...
      if (trackList.length < max) max = trackList.length;

      const results = trackList.slice(0, max);
