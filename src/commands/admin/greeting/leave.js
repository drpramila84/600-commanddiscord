    ...
    subcommands: [
      {
        ...
        description: "enable or disable leave message",
      },
      {
      ...
      {
        trigger: "preview",
        description: "preview the configured leave message",
      },
      {
        trigger: "desc <text>",
...
async function sendPreview(settings, member) {
  if (!settings.farewell?.enabled) return "Leave message not enabled in this server";

  const targetChannel = member.guild.channels.cache.get(settings.farewell.channel);
  if (!targetChannel) return "No channel is configured to send leave message";

  const response = await buildGreeting(member, "FAREWELL", settings.farewell);
  await targetChannel.safeSend(response);

  return `Sent leave preview to ${targetChannel.toString()}`;
}
...
async function setStatus(settings, status) {
  ...
  await settings.save();
  return `Configuration saved! Leave message ${enabled ? "enabled" : "disabled"}`;
}
...
async function setChannel(settings, channel) {
  ...
  await settings.save();
  return `Configuration saved! Leave message will be sent to ${channel ? channel.toString() : "Not found"}`;
}

async function setDescription(settings, desc) {
  settings.farewell.embed.description = desc;
  await settings.save();
  return "Configuration saved! Leave message updated";
}
