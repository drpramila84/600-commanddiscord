...
module.exports = (client) => {
  ...
  if (SUPPORT_SERVER) {
    components.push(new ButtonBuilder().setLabel("https://discord.gg/es6kwz26My").setURL(SUPPORT_SERVER).setStyle(ButtonStyle.Link));
  }

  if (DASHBOARD.enabled) {
    components.push(
      new ButtonBuilder().setLabel("DashboardLink").setURL(DASHBOARD.baseURL).setStyle(ButtonStyle.Link)
    );
  }
