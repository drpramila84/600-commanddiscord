...
async function performAutomod(message, settings) {
  ...
  if (automod.anti_spam) {
    ...
    if (antispamCache.has(key)) {
      ...
      const timeDiff = Date.now() - antispamInfo.timestamp;
      
      // Check for duplicate content spam (same message sent quickly)
      const isDuplicateContent = antispamInfo.content === content && timeDiff < MESSAGE_SPAM_THRESHOLD;
      
      // Check for rapid message spam (too many messages too fast)
      const isRapidSpam = timeDiff < 1000; // Less than 1 second between messages
      
      // Check for cross-channel spam (same content in different channels)
      const isCrossChannelSpam = antispamInfo.channelId !== message.channelId && 
                                  antispamInfo.content === content && 
                                  timeDiff < MESSAGE_SPAM_THRESHOLD;
      
      if (isDuplicateContent || isRapidSpam || isCrossChannelSpam) {
        // Increment spam count
        antispamInfo.count = (antispamInfo.count || 1) + 1;
        antispamInfo.timestamp = Date.now();
        antispamCache.set(key, antispamInfo);
        
        // Trigger after 3 rapid messages or any duplicate/cross-channel spam
        if (antispamInfo.count >= 3 || isDuplicateContent || isCrossChannelSpam) {
          fields.push({ name: "AntiSpam Detection", value: `Spam detected (${antispamInfo.count} messages)`, inline: true });
          shouldDelete = true;
          strikesTotal += 1;
        }
      } else {
        // Reset counter for normal messages
